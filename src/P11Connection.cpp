...
void P11Connection::init_shortcuts() 
{
...
P11Connection::P11Connection(SpikingGroup * source, NeuronGroup * destination, TransmitterType transmitter) : DuplexConnection(source, destination, transmitter)
{

}

P11Connection::P11Connection(SpikingGroup * source, NeuronGroup * destination, 
		const char * filename, 
		AurynFloat eta, 
		AurynFloat kappa, 
...
inline AurynWeight P11Connection::dw_post(const NeuronID pre, const NeuronID post, const AurynWeight * w, const AurynWeight w0)
{
...
void P11Connection::evolve()
{
	...
	auryn_vector_float_saxpy( dt/tau_hom, state_temp, hom );
...
void P11Connection::consolidate() {
	for (AurynLong i = 0 ; i < w_solid_matrix->get_nonzero() ; ++i ) {
		AurynWeight * fragile = w->get_data_begin()+i;
		AurynWeight * solid   = w_solid_matrix->get_data_begin()+i;
		*solid = *fragile;
...
void P11Connection::push_attributes()
{
...
void P11Connection::set_tau_f(AurynFloat tauf) {
	tau_f = tauf;
}

void P11Connection::set_tau_d(AurynFloat taud) {
	tau_d = taud;
}

void P11Connection::set_ujump(AurynFloat r) {
	Ujump = r;
}

void P11Connection::set_urest(AurynFloat r) {
	Urest = r;
}

void P11Connection::set_weight_a(AurynFloat w0) {
	weight_a = w0;
	weight_b = (weight_a+weight_c)/2;
}

void P11Connection::set_weight_c(AurynFloat w2) {
	weight_c = w2;
	weight_b = (weight_a+weight_c)/2;
}

void P11Connection::set_beta(AurynFloat b)
{
	beta = b;
	beta_fudge = beta; 
...
void P11Connection::load_fragile_matrix(string filename)
{
